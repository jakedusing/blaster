	...
	if (MuzzleFlashSocket)
	{
		...
		for (FVector_NetQuantize HitTarget : HitTargets)
		{
			...
			if (BlasterCharacter)
			{
				...
				if (bHeadShot)
				{
					// If character already exists in Headshothitmap, increment.. otherwise add them with 1 hit
					if (HeadShotHitMap.Contains(BlasterCharacter)) HeadShotHitMap[BlasterCharacter]++;
					else HeadShotHitMap.Emplace(BlasterCharacter, 1);
				}
				else
				{
					// If character already exists in hitmap, increment.. otherwise add them with 1 hit
